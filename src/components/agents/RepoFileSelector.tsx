...
export default function RepoFileSelector({
  ...
  return <div>
      <form onSubmit={handleGithubRepoInput} className="mb-2 flex gap-2 items-center relative">
        <Input value={githubUrl} onChange={e => setGithubUrl(e.target.value)} placeholder="Enter GitHub repository URL (public repo)..." className="text-base flex-1" />
        <Button type="submit" variant="secondary" disabled={loadingFiles}>
          {loadingFiles ? <>
      ...
      <div className="flex gap-2 mb-4 items-center">
        ...
        {githubToken && <Button type="button" size="sm" variant="outline" className="flex items-center gap-1 px-2 border-red-500 hover:bg-red-950/10" onClick={handleClearGithubToken}>
      ...
      {fetchError && <Alert variant="destructive" className="mb-4 py-2 bg-destructive/10 border-destructive/30">
          <AlertCircle className="w-4 h-4" />
          <AlertDescription className="text-sm ml-2">{fetchError}</AlertDescription>
        </Alert>}

      {repoFiles.length > 0 && <div className="mb-4">
          <Button type="button" variant="outline" size="sm" className="flex items-center gap-2" onClick={() => setFileDropdownOpen(open => !open)} aria-expanded={fileDropdownOpen}>
            <ArrowDown className={`transition-transform ${fileDropdownOpen ? "rotate-180" : ""}`} />
            <span>
              {selectedFile ? <span className="flex items-center gap-2"><File className="w-4 h-4" /> {selectedFile.path}</span> : "Select file from repo"}
            </span>
          </Button>
          {fileDropdownOpen && <div className="max-h-64 overflow-auto rounded border bg-popover mt-2 shadow z-10 absolute w-[400px]">
              {repoFiles.map(entry => <div key={entry.path} tabIndex={0} className={`flex items-center px-3 py-2 cursor-pointer hover:bg-squadrun-primary/10 ${selectedFile?.path === entry.path ? "bg-squadrun-primary/20" : ""}`} onClick={() => {
          ...
          {fetchingFileContent && <div className="text-xs text-squadrun-gray mt-1 flex items-center">
